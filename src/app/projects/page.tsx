      ...
      <section id="projects" className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
          <div className="space-y-4">
            <Badge variant="outline" className="text-primary tracking-[0.3em] uppercase">Portfolio</Badge>
        ...
        <div className="grid md:grid-cols-2 gap-10">
          {projects.map((p) => (
            <Card key={p.id} className="group overflow-hidden border-border bg-card/40 hover:bg-card/60 transition-all duration-500 hover:shadow-2xl hover:border-primary/20">
              <div className="relative h-72 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-background/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-8">
                  <Button size="lg" className="rounded-full font-bold shadow-xl" asChild>
              ...
              <CardHeader className="p-8">
                <div className="flex flex-wrap gap-2 mb-4">
                  {p.tech.slice(0, 3).map(t => (
                    <Badge key={t} variant="secondary" className="text-[10px] uppercase font-bold tracking-tighter bg-primary/5 text-primary border-primary/10">{t}</Badge>
                  ))}
                </div>
              ...
              <CardContent className="px-8 pb-8 pt-0">
                <Dialog>
                  <DialogTrigger asChild>
                    <Button variant="outline" className="w-full rounded-xl font-headline font-bold uppercase tracking-widest text-xs h-12">
                      Case Study Details
                    </Button>
                  </DialogTrigger>
                  <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto rounded-3xl border-border bg-card">
                    <DialogHeader>
                      <DialogTitle className="text-4xl font-headline font-bold mb-6 text-foreground">{p.title}</DialogTitle>
                    </DialogHeader>
                    <div className="grid md:grid-cols-2 gap-12">
                      <div className="space-y-8">
                        <img src={p.image} alt={p.title} className="w-full rounded-2xl shadow-2xl border border-border" />
                        <div className="flex flex-wrap gap-2">
                          {p.tech.map(t => <Badge key={t} variant="secondary" className="bg-muted text-foreground uppercase text-[10px] font-bold">{t}</Badge>)}
                        </div>
                      </div>
                      <div className="space-y-8">
                        <div>
                        ...
                        <Button className="w-full rounded-2xl font-headline font-bold h-14" asChild>
                          <a href={p.link} target="_blank" rel="noopener noreferrer">Visit Deployment <ExternalLink className="ml-2 w-4 h-4" /></a>
                        </Button>
                      </div>
                    </div>
                  </DialogContent>
